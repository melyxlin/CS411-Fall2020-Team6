...
.then(client => {
  ...
  router.get('/getGif/:search_term', (req, res) => {
    request('https://api.giphy.com/v1/gifs/search?api_key=' + process.env.GIPHY 
    + '&limit=1&q=' + req.params.search_term +'&lang=en', function (error, response, body) {
      if (!error && response.statusCode == 200) {
        res.send(JSON.parse(body).data[0].embed_url)
      }
    })
  })
